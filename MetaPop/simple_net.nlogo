...
;; All set up and ready to form links. BUT DO READ THE COMMENTS FOR THE SETUP-GIS_WORLD FUNCTION!!
end
...
How to import centroids and polygons from shapefiles and form a netowrk based on known distance between centroids.

## HOW IT WORKS

Uses the GIS extension. 

## HOW TO USE IT

Make sure that the model and the shapefiles are in the same directory ( or change the path accordingly)

## THINGS TO NOTICE

Notice the way that the size of the netlogo world is adjusted so that pach sizes correspond to a known geographical scale.

## THINGS TO TRY

Change the distance used to setup the network

## EXTENDING THE MODEL

This only loads data. Not a model as such.
